...
const TabOptions = ({activeTab, setTab}) => {
    return (
        ...
        <div className = "max-width options_wrapper">
            {
                tabs.map((tab) => {
                    return (
                    <div onClick = {() => setTab(tab.name)}
                    className = {`tab_item absolute_center cur_poi ${activeTab === tab.name && "active_tab"}`}
                    >
                        <div className = "tab_image_container absolute_center" 
